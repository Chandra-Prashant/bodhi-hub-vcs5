...
def project(client, headers):
    """Documents now belong to a project, so every test needs one."""
...
def _upload(client, headers, project, name="project.txt",
            body=b"Aligarh Solar One, 50 MW."):
    return client.post(f"/api/v1/documents/upload?project_id={project}",
                       headers=headers,
                       files={"file": (name, body, "text/plain")})


def _documents(client, headers, project):
    return client.get(f"/api/v1/documents?project_id={project}", headers=headers)


def _queue(client, headers, project):
    return client.get(f"/api/v1/documents/queue?project_id={project}",
                      headers=headers)
...
def test_a_clean_upload_is_approved_without_review(
        client, headers, project, stub_extractor, local_storage):
    response = _upload(client, headers, project)
    assert response.status_code == 201, response.text
    body = response.json()
...
def test_a_flagged_upload_needs_review(
    ...
    body = _upload(client, headers, project).json()
    assert body["document"]["status"] == "NEEDS_REVIEW"
    assert any(i["field_name"] == "country_iso2" for i in body["review_items"])


def test_an_unsupported_type_is_rejected(client, headers, project, local_storage):
    response = client.post(
        f"/api/v1/documents/upload?project_id={project}", headers=headers,
        files={"file": ("archive.zip", b"\x00\x01", "application/zip")})
    assert response.status_code == 400
    assert "not accepted" in response.json()["detail"]


def test_an_empty_upload_is_rejected(client, headers, project, local_storage):
    response = client.post(f"/api/v1/documents/upload?project_id={project}",
                           headers=headers,
...
def test_the_same_file_cannot_be_uploaded_twice(
        client, headers, project, stub_extractor, local_storage):
    _upload(client, headers, project)
    again = _upload(client, headers, project, name="different-name.txt")
...
def test_documents_are_scoped_to_the_organization(
        client, headers, project, auth_headers, make_user, stub_extractor, local_storage):
    _upload(client, headers, project)
    ...
    assert len(_documents(client, headers, project).json()) == 1
    assert _documents(client, headers=other_headers, project=project).json() == []
...
def test_the_queue_holds_only_pending_items(
    ...
    queue = _queue(client, headers, project).json()
    assert len(queue) == 1
...
def test_the_queue_puts_blocking_items_first(
    ...
    severities = [i["severity"] for i in _queue(client, headers, project).json()]
...
def _first_queue_item(client, headers, project):
    return _queue(client, headers, project).json()[0]
...
def _document_id(client, headers, project):
    return _documents(client, headers, project).json()[0]["id"]
...
def test_assess_runs_on_a_clean_document(
    ...
    _upload(client, headers, project)
    response = client.post(
        f"/api/v1/documents/{_document_id(client, headers, project)}/assess",
        headers=headers)
...
def test_assess_is_scoped_to_the_organization(
        client, headers, project, auth_headers, make_user, stub_extractor, local_storage):
    _upload(client, headers, project)
    document_id = _document_id(client, headers, project)
...
def test_delete_removes_the_document(
    ...
    assert _documents(client, headers, project).json() == []
...
def test_project_managers_cannot_delete(
        client, headers, project, auth_headers, make_user, stub_extractor,
        local_storage):
    _upload(client, headers, project)
    document_id = _document_id(client, headers, project)
...
def test_deleting_removes_the_review_items_too(
    ...
    client.delete(f"/api/v1/documents/{_document_id(client, headers, project)}",
                  headers=headers)
    assert _queue(client, headers, project).json() == []
...
def test_the_extraction_endpoint_reports_a_missing_document(client, headers, project):
    import uuid
...
def test_a_bad_upload_is_refused_without_a_model_configured(client, headers, project,
                                                            local_storage):
...
def test_assessing_a_document_does_not_return_esg_entries(client, headers, project,
                                                          stub_extractor,
                                                          local_storage):
    """The handover payload is built from the document, which never contains
    ESG judgements. The frontend must merge this onto current state rather
    than replace it — spreading it over a blank project discarded twelve
    categories a person had typed, and the debounced save then wrote the empty
    version over the stored one.

    This test pins the contract the frontend depends on: the payload carries
    no esg_entries, so the caller is responsible for preserving them.
    """
    _upload(client, headers, project, "p.txt")
    document_id = _document_id(client, headers, project)
    body = client.post(f"/api/v1/documents/{document_id}/assess",
                       headers=headers).json()

    assert not body["project"].get("esg_entries")
