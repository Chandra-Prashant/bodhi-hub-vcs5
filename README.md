...
**595 tests passing** — 568 domain, 27 endpoint.

> **Not yet run against a real model.** Every verification used stubbed
> extraction and narrative models. Extraction quality on a real PDF, reading a
> photographed form, and whether the narrative model respects the placeholder
> rule in practice are all unproven. Set `GEMINI_API_KEY` and put one real
...
**1. Upload** — PDF, Word, Excel, CSV or an image of a form. Stored with a
SHA-256 hash, deduplicated per organization, filename sanitised, 25 MB cap.

**2. Extract** — into `ProjectExtraction`, a fixed Pydantic schema where every
field carries a value, a confidence band, the page it came from and the
sentence it was taken from. Images go to the model as vision input rather than
through a separate OCR service: Rules.md already grants the LLM "extracting
structured data from documents", and it is one fewer hop where a digit can
change.
...
**5. Assess** — `POST /documents/{id}/assess`. Reviewer corrections override
extracted values, rejected values are dropped, and unresolved blocking items
refuse the handover with a 409. Technology strings map through an explicit
keyword table with disqualifying terms; "solar thermal" is refused rather than
matched to solar PV, because CSP is not a VMR0017 Table 1 type and the wrong
match selects the wrong eligibility rules.
...
Expect `595 passed`. `568 passed, 27 skipped` means Postgres is not reachable —
...
app/
  ...
  services/     ingestion, handover, docx rendering, auditor, audit trail
...
`POST /documents/{id}/assess` has no button yet — the loop is closeable by API
...
1. **Never run against a real model.** See the note at the top.
...
4. **No assess button in the UI.** The route exists; nothing calls it.
5. **Nothing is deployed.** `DEPLOY.md` covers VPS, Render, Railway and Fly.
6. **No file storage backend.** Architecture.md specifies S3-compatible object
   storage; uploads land on the application volume, behind
...
column renders `pgvector.sqlalchemy.vector.VECTOR(dim=768)` and fails with
...
were lost once to a file overwrite.
